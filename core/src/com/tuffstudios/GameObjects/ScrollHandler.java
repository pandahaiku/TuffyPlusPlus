...
public class ScrollHandler {
	...
	public boolean collides(Elephant elephant) {
		if (!pipe1.isScored() && pipe1.getX() + (pipe1.getWidth() / 2) < elephant.getX() + elephant.getWidth()) {
			addScore(1);
			pipe1.setScored(true);
			AssetLoader.coin.play(AssetLoader.VOLUME);
		} else if (!pipe2.isScored() && pipe2.getX() + (pipe2.getWidth() / 2) < elephant.getX() + elephant.getWidth()) {
			addScore(1);
			pipe2.setScored(true);
			AssetLoader.coin.play(AssetLoader.VOLUME);
		} else if (!pipe3.isScored() && pipe3.getX() + (pipe3.getWidth() / 2) < elephant.getX() + elephant.getWidth()) {
			addScore(1);
			pipe3.setScored(true);
			AssetLoader.coin.play(AssetLoader.VOLUME);
		}

		return (pipe1.collides(elephant) || pipe2.collides(elephant) || pipe3.collides(elephant));
	}
